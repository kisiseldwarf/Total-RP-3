...
local OpenProfileAction = Ellyb.Class("OpenProfileAction", UnitAction);

function OpenProfileAction:initialize()
	UnitAction.initialize(self, loc.UP_OPEN_PROFILE_BUTTON);
end

function OpenProfileAction:OnTriggered(_, name, server)
	TRP3_API.slash.openProfile(utils.str.unitInfoToID(name, server));
end

-- Exports...
UnitPopups.OpenProfileAction = OpenProfileAction;
