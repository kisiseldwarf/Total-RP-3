...
local saveCurrentProfileID, saveClientInformation, saveInformation =
	TRP3_API.register.saveCurrentProfileID, TRP3_API.register.saveClientInformation, TRP3_API.register.saveInformation
...
function createVernumQuery()
	...
	local secondaryPetLine, secondaryPetV1, secondaryPetV2 =
		TRP3_API.companions.player.getCurrentSecondaryPetQueryLine()
...
local function checkVersion(sender, senderVersion, senderVersionText, extendedVersion, extendedVersionText)
	...
	if
		extendedVersion
		and extendedVersionText
		and Globals.extended_version
		and extendedVersion > Globals.extended_version
		and not has_seen_extended_update_alert
	then
...
local function incomingVernumQuery(structure, senderID, sendBack)
	...
	saveClientInformation(
		...
		senderVersionText,
		false,
		senderExtendedVersion,
		senderIsTrial,
		senderExtendedVersionText
	)
	...
	TryQueryInformation(
		...
		structure[VERNUM_QUERY_INDEX_CHARACTER_CHARACTERISTICS_V]
	)
...
local function incomingInformationType(informationType, senderID)
	...
	if data then
		...
		AddOn_TotalRP3.Communications.sendObject(
			prefix,
			object,
			channel,
			target,
...
local function incomingInformationTypeSent(structure, senderID, channel)
	...
	if
		informationType == registerInfoTypes.CHARACTERISTICS
		or informationType == registerInfoTypes.ABOUT
		or informationType == registerInfoTypes.MISC
		or informationType == registerInfoTypes.CHARACTER
	then
...
function TRP3_API.register.inits.dataExchangeInit()
	...
	TRP3_API.events.listenToEvent(TRP3_API.events.MOUSE_OVER_CHANGED, function(targetID, targetMode)
		...
		elseif
			(targetMode == TRP3_Enums.UNIT_TYPE.BATTLE_PET or targetMode == TRP3_Enums.UNIT_TYPE.PET) and targetID
		then
	...
	AddOn_TotalRP3.Communications.broadcast.registerCommand(
		Comm.broadcast.HELLO_CMD,
		function(sender, version, versionDisplay, extendedVersion, extendedVersionDisplay)
...
function TRP3_API.slash.openProfile(...)
	...
	if
		characterToOpen == TRP3_API.globals.player_id
		or (isUnitIDKnown(characterToOpen) and hasProfile(characterToOpen))
	then
...
Events.listenToEvent(Events.REGISTER_DATA_UPDATED, function(unitID, _, dataType)
	if AddOn_Chomp.InsensitiveStringEquals(characterToOpen, unitID) and (not dataType or dataType == "character") then
