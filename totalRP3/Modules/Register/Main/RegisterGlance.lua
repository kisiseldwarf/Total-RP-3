...
local tostring, _G, pairs, type, tinsert, assert, wipe, bit, strsplit =
	tostring, _G, pairs, type, tinsert, assert, wipe, bit, strsplit
...
local function getSlotPresetDataForList(output)
	...
	for _, category in pairs(tmp) do
		...
		for _, glance in pairs(categoryTab) do
			...
			tinsert(categoryListElementTarget[2], {
				getIcon(glanceInfo.icon, 20) .. " " .. tostring(glanceInfo.title),
				{
...
local function getGlanceMenuClipboardEntries(button, output)
	...
	local clipboardType = (button.targetType == TRP3_Enums.UNIT_TYPE.CHARACTER) and GLANCE_MENU_CLIPBOARD_ID_CHARACTER
	...
	if button.data then
		local copyKey =
			string.format("%s:%s,%s", GLANCE_MENU_ENTRY_CLIPBOARD_COPY_PREFIX, clipboardType, button:GetName())
	...
	TRP3_AtFirstGlanceEditorIcon:SetScript("OnClick", function(self)
		...
		if self.isExternal then
			TRP3_API.popup.showPopup(
				...
				{ parent = TRP3_AtFirstGlanceEditor, point = "RIGHT", parentPoint = "LEFT" },
				{ onIconSelected, nil, 0.75 }
			)
		else
...
local hasProfile, isUnitIDKnown, getUnitIDCurrentProfile =
	TRP3_API.register.hasProfile, TRP3_API.register.isUnitIDKnown, TRP3_API.register.getUnitIDCurrentProfile
local getCompanionProfile, getCompanionRegisterProfile =
	TRP3_API.companions.player.getCompanionProfile, TRP3_API.companions.register.getCompanionProfile
...
local function onGlanceSelection(presetAction, button)
	...
	if type(presetAction) == "string" then
		...
		if action == LOAD_PREFIX then
			...
			getOnGlanceEditorConfirmFunction(button)(
				button.slot,
				preset.icon,
				true,
				preset.title,
...
local function onGlanceSlotClick(button, clickType)
	if clickType == "LeftButton" and button.isCurrentMine then
		if IsShiftKeyDown() then
			...
			if TRP3_AtFirstGlanceEditor:IsVisible() and TRP3_AtFirstGlanceEditor.current == button then
				...
				openGlanceEditor(
					button.slot,
					button.data or button.glanceTab[button.slot] or {},
					getOnGlanceEditorConfirmFunction(button),
					TRP3_AtFirstGlanceEditor,
					button.targetID,
					button.profileID
				)
			end
	...
	elseif clickType == "RightButton" then
		...
		local onSelected = function(value)
			onGlanceSelection(value, button)
		end
		...
		for i = 1, 5, 1 do
			...
			if glance and glance.AC then
				...
				setTooltipForSameFrame(
					...
					0,
					0,
					Utils.str.icon(icon, 30) .. " " .. glanceTitle,
					TTText
				)
			else
...
local function onStart()
	...
	local function replaceBar()
		...
		ui_GlanceBar:SetPoint(
			"BOTTOM",
			parentFrame,
			"BOTTOM",
			getConfigValue(CONFIG_GLANCE_ANCHOR_X) / parentScale,
			getConfigValue(CONFIG_GLANCE_ANCHOR_Y) / parentScale
		)
	end
