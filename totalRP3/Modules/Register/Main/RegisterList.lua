...
local registerMenu, selectMenu, openMainFrame =
	TRP3_API.navigation.menu.registerMenu, TRP3_API.navigation.menu.selectMenu, TRP3_API.navigation.openMainFrame
...
local getIgnoredList, unignoreID, isIDIgnored =
	TRP3_API.register.getIgnoredList, TRP3_API.register.unignoreID, TRP3_API.register.isIDIgnored
local getRelationText, getRelationTooltipText =
	TRP3_API.register.relation.getRelationText, TRP3_API.register.relation.getRelationTooltipText
local unregisterMenu = TRP3_API.navigation.menu.unregisterMenu
local displayDropDown, showAlertPopup, showConfirmPopup =
	TRP3_API.ui.listbox.displayDropDown, TRP3_API.popup.showAlertPopup, TRP3_API.popup.showConfirmPopup
local showTextInputPopup = TRP3_API.popup.showTextInputPopup
local deleteProfile, deleteCharacter, getProfileList =
	TRP3_API.register.deleteProfile, TRP3_API.register.deleteCharacter, TRP3_API.register.getProfileList
...
local function openPage(profileID, unitID)
	...
	if isMenuRegistered(menuID) then
		...
		registerMenu({
			...
			text = tabText,
			onSelected = function()
				setPage("player_main", pageContext)
			end,
		...
		if
			(unitID and unitIDIsFilteredForMatureContent(unitID))
			or (profileID and profileIDISFilteredForMatureContent(profileID))
		then
...
local function openCompanionPage(profileID)
	...
	if isMenuRegistered(currentlyOpenedProfilePrefix .. profileID) then
		...
		registerMenu({
			...
			onSelected = function()
				setPage(
					TRP3_API.navigation.page.id.COMPANIONS_PAGE,
					{ profile = profile, profileID = profileID, isPlayer = false }
				)
			end,
			isChildOf = REGISTER_PAGE,
			closeable = true,
...
local comparators = {
	nameComparator,
	nameComparatorInverted,
	infoComparator,
	infoComparatorInverted,
	timeComparator,
	timeComparatorInverted,
}
...
local function decorateCharacterLine(line, characterIndex)
	...
	if relation:len() > 0 then
		...
		setTooltipForSameFrame(
			_G[line:GetName() .. "ClickMiddle"],
			"TOPLEFT",
			0,
			5,
			middleTooltipTitle,
			color .. middleTooltipText
		)
	else
	...
	if #rightTooltipTexts > 0 then
		setTooltipForSameFrame(
			_G[line:GetName() .. "ClickRight"],
			"TOPLEFT",
			0,
			5,
			loc.REG_LIST_FLAGS,
			table.concat(rightTooltipTexts, "\n")
		)
	else
	...
	for profileID, profile in pairs(profileList) do
		...
		if profile.characteristics and not Ellyb.Tables.isEmpty(profile.characteristics) then
			...
			for unitID, _ in pairs(profile.link or Globals.empty) do
				...
				if
					characterData
					and characterData.guild
					and string.find(characterData.guild:lower(), guildSearch, 1, true)
				then
					guildIsConform = true
				end
...
local function onCharactersActionSelected(value)
	...
	if value == "purge_time" then
		...
		if #profilesToPurge == 0 then
			...
			showConfirmPopup(
				loc.REG_LIST_ACTIONS_PURGE_TIME_C:format(loc.REG_LIST_ACTIONS_PURGE_COUNT:format(#profilesToPurge)),
				function()
	...
	elseif value == "purge_unlinked" then
		...
		if #profilesToPurge == 0 then
			...
			showConfirmPopup(
				loc.REG_LIST_ACTIONS_PURGE_UNLINKED_C:format(loc.REG_LIST_ACTIONS_PURGE_COUNT:format(#profilesToPurge)),
				function()
					for _, profileID in pairs(profilesToPurge) do
						deleteProfile(profileID, true)
					end
	...
	elseif value == "purge_ignore" then
		...
		if #profilesToPurge + #characterToPurge == 0 then
			...
			showConfirmPopup(
				loc.REG_LIST_ACTIONS_PURGE_IGNORE_C:format(
					loc.REG_LIST_ACTIONS_PURGE_COUNT:format(#profilesToPurge + #characterToPurge)
				),
				function()
					for _, profileID in pairs(profilesToPurge) do
						deleteProfile(profileID, true)
...
local function onCharactersActions(self)
	...
	if tsize(selectedIDs) > 0 then
		tinsert(values, {
			loc.REG_LIST_ACTIONS_MASS:format(tsize(selectedIDs)),
			{
...
local companionIDToInfo, getAssociationsForProfile =
	TRP3_API.utils.str.companionIDToInfo, TRP3_API.companions.register.getAssociationsForProfile
...
local function getCompanionLines()
	...
	for profileID, profile in pairs(profiles) do
		...
		if
			...
			and profile.data
			and profile.data.NA
			and string.find(profile.data.NA:lower(), nameSearch, 1, true)
		then
			nameIsConform = true
		end
...
local function decorateIgnoredLine(line, unitID)
	...
	setTooltipForSameFrame(
		...
		5,
		unitID,
		loc.REG_LIST_IGNORE_TT:format(getIgnoredList()[unitID])
	)
	setTooltipForSameFrame(_G[line:GetName() .. "ClickMiddle"])
	setTooltipForSameFrame(_G[line:GetName() .. "ClickRight"])
end
...
TRP3_API.events.listenToEvent(TRP3_API.events.WORKFLOW_ON_LOAD, function()
	...
	Events.listenToEvent(Events.REGISTER_DATA_UPDATED, function(unitID, _, dataType)
		if
			TRP3_MainFrame:IsShown()
			and getCurrentPageID() == REGISTER_LIST_PAGEID
			and unitID ~= Globals.player_id
			and (not dataType or dataType == "characteristics")
		then
			refreshList()
		end
	end)
	...
	setTooltipForSameFrame(
		TRP3_RegisterListCharactFilterButton,
		"LEFT",
		0,
		5,
		loc.REG_LIST_FILTERS,
	...
	TRP3_API.ui.frame.setupEditBoxesNavigation({
		TRP3_RegisterListFilterCharactName,
		TRP3_RegisterListFilterCharactGuild,
	})
	...
	setTooltipForSameFrame(
		TRP3_RegisterListPetFilterButton,
		"LEFT",
		0,
		5,
		loc.REG_LIST_FILTERS,
	...
	TRP3_API.ui.frame.setupEditBoxesNavigation({
		TRP3_RegisterListPetFilterName,
		TRP3_RegisterListPetFilterType,
		TRP3_RegisterListPetFilterMaster,
	})
			...
			adapter = function(buttonStructure, unitID)
				...
				if unitID ~= Globals.player_id and hasProfile(unitID) then
					...
					if profile.about and not profile.about.read then
						buttonStructure.tooltipSub = "|cff00ff00"
							.. loc.REG_TT_NOTIF
							.. "\n"
