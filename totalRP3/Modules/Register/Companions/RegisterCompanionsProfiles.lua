...
local showAlertPopup, showTextInputPopup, showConfirmPopup =
	TRP3_API.popup.showAlertPopup, TRP3_API.popup.showTextInputPopup, TRP3_API.popup.showConfirmPopup
local handleMouseWheel = TRP3_API.ui.list.handleMouseWheel
local initList = TRP3_API.ui.list.initList
local getProfiles, isProfileNameAvailable =
	TRP3_API.companions.player.getProfiles, TRP3_API.companions.player.isProfileNameAvailable
...
local TRP3_CompanionsProfilesList, TRP3_CompanionsProfilesListSlider, TRP3_CompanionsProfilesListEmpty =
	TRP3_CompanionsProfilesList, TRP3_CompanionsProfilesListSlider, TRP3_CompanionsProfilesListEmpty
local EMPTY, PetCanBeRenamed = Globals.empty, PetCanBeRenamed
local getCompanionProfile, getCompanionProfileID =
	TRP3_API.companions.player.getCompanionProfile, TRP3_API.companions.player.getCompanionProfileID
...
local isTargetTypeACompanion, companionHasProfile =
	TRP3_API.ui.misc.isTargetTypeACompanion, TRP3_API.companions.register.companionHasProfile
local getCompanionNameFromSpellID = TRP3_API.companions.getCompanionNameFromSpellID
local getCurrentMountSpellID, getCurrentMountProfile =
	TRP3_API.companions.player.getCurrentMountSpellID, TRP3_API.companions.player.getCurrentMountProfile
		...
		registerMenu({
			...
			onSelected = function()
				setPage(
					...
					{ profile = profile, profileID = profileID, isPlayer = true }
				)
			end,
...
local function uiCreateProfile()
	showTextInputPopup(loc.PR_PROFILEMANAGER_CREATE_POPUP, function(newName)
		if newName and #newName ~= 0 then
			if not uiCheckNameAvailability(newName) then
				return
			end
			local profileID = createProfile(newName)
			openProfile(profileID)
		end
	end, nil, loc.PR_CO_NEW_PROFILE)
end
...
local function uiDeleteProfile(profileID)
	showConfirmPopup(
		loc.PR_CO_PROFILEMANAGER_DELETE_WARNING:format(
			Utils.str.color("g") .. getProfiles()[profileID].profileName .. "|r"
		),
		function()
...
local function uiBoundProfile(profileID, companionType)
	TRP3_API.popup.showCompanionBrowser(function(companionInfo)
		ui_boundPlayerCompanion(
			companionInfo[5] and tostring(companionInfo[5]) or companionInfo[1],
			profileID,
			companionType
		)
...
local function uiUnboundTargetProfile(_, companionInfo)
	local companionID, companionType =
		companionInfo:sub(1, companionInfo:find("|") - 1), companionInfo:sub(companionInfo:find("|") + 1)
	unboundPlayerCompanion(companionID, companionType)
	TRP3_API.ui.tooltip.toast(
		loc.REG_COMPANION_LINKED_NO:format("|cff00ff00" .. getCompanionNameFromSpellID(companionID) .. "|r"),
		4
	)
	uiInitProfileList()
end
...
local function decorateProfileList(widget, index)
	...
	for companionID, companionType in pairs(profile.links or EMPTY) do
		listText = listText
			...
			.. "|cffff9900 ("
			.. getCompanionTypeText(companionType)
			.. ")|r\n"
		i = i + 1
	end
...
function uiInitProfileList()
	...
	for profileID, _ in pairs(profiles) do
		if
			not profileSearch or string.find(profiles[profileID].profileName:lower(), profileSearch:lower(), 1, true)
		then
...
ui_boundPlayerCompanion = function(companionID, profileID, targetType)
	...
	displayMessage(
		loc.REG_COMPANION_LINKED:format(
			"|cff00ff00" .. companionName .. "|r",
			"|cff00ff00" .. profile.profileName .. "|r"
		)
	)
...
local function createNewAndBound(companionID, targetType)
	...
	end, nil, getCompanionNameFromSpellID(companionID))
end
...
local function companionProfileSelectionList(unitID, targetType, _, button)
	...
	if ownerID == Globals.player_id then
		...
		displayDropDown(button, list, function(value)
			onCompanionProfileSelection(value, companionID, targetType)
		end, 0, true)
	else
...
TRP3_API.events.listenToEvent(TRP3_API.events.WORKFLOW_ON_LOAD, function()
	...
	for i = 1, 5 do
		...
		Ellyb.Tooltips
			.getTooltip(_G[widget:GetName() .. "Action"])
			...
			adapter = function(buttonStructure, unitID)
				...
				if unitID == Globals.player_id then
					...
					if profile then
						...
						buttonStructure.tooltipSub = "|cffffff00"
							.. loc.CM_CLICK
							.. ": |r"
							.. loc.REG_COMPANION_TF_BOUND_TO
