...
local getCurrentContext, getCurrentPageID =
	TRP3_API.navigation.page.getCurrentContext, TRP3_API.navigation.page.getCurrentPageID
local getPlayerCurrentProfileID, isProfileNameAvailable, createProfile, selectProfile =
	TRP3_API.profile.getPlayerCurrentProfileID,
	TRP3_API.profile.isProfileNameAvailable,
	TRP3_API.profile.createProfile,
	TRP3_API.profile.selectProfile
...
local function deleteCharacter(unitID)
	...
	if
		characters[unitID].profileID
		and profiles[characters[unitID].profileID]
		and profiles[characters[unitID].profileID].link
	then
...
local function unitIDIsFilteredForMatureContent(unitID)
	if
		not TRP3_API.register.mature_filter
		or not unitID
		or unitID == Globals.player_id
		or not isUnitIDKnown(unitID)
		or not profileExists(unitID)
	then
		return false
	end
	...
	return profile.hasMatureContent
		and getConfigValue("register_mature_filter")
		and not (TRP3_API.register.mature_filter.isProfileSafeListed(profileID))
end
...
local function profileIDISFilteredForMatureContent(profileID)
	if not TRP3_API.register.mature_filter then
		return false
	end
...
--- Raises error if unknown unitName
function TRP3_API.register.saveClientInformation(
	unitID,
	client,
	clientVersion,
	msp,
...
local function sanitizeFullProfile(data)
	...
	if
		data.player.characteristics
		and TRP3_API.register.sanitizeProfile(registerInfoTypes.CHARACTERISTICS, data.player.characteristics)
	then
		somethingWasSanitizedInsideProfile = true
	end
	if
		data.player.character and TRP3_API.register.sanitizeProfile(registerInfoTypes.CHARACTER, data.player.character)
	then
		somethingWasSanitizedInsideProfile = true
	end
	...
	if getCurrentPageID() == "player_main" then
		...
		if not context.isPlayer and profileID == context.profileID then
			if infoType == registerInfoTypes.ABOUT and tabGroup.current == 2 then
				showAboutTab()
			elseif
				(infoType == registerInfoTypes.CHARACTERISTICS or infoType == registerInfoTypes.CHARACTER)
				and tabGroup.current == 1
			then
...
local function createTabBar()
	...
	tabGroup = TRP3_API.ui.frame.createTabPanel(
		frame,
		{
		...
		function(callback)
			if getCurrentContext() and getCurrentContext().isEditMode then
				TRP3_API.popup.showConfirmPopup(loc.REG_PLAYER_CHANGE_CONFIRM, function()
					callback()
				end)
			else
				callback()
...
local function cleanupCharacters()
	for unitID, character in pairs(characters) do
		if
			character.profileID
			and (
				not profiles[character.profileID]
				or not profiles[character.profileID].link
				or not profiles[character.profileID].link[unitID]
			)
		then
...
local function cleanupProfiles()
	...
	for profileID, profile in pairs(profiles) do
		if
			not protectedProfileIDs[profileID]
			and (not profile.time or time() - profile.time > getConfigValue("register_auto_purge_mode"))
		then
...
function TRP3_API.register.init()
	...
	TRP3_RegisterDefaultViewCreateProfile:SetScript("OnClick", function()
		...
		end, nil, Globals.player_realm .. " - " .. Globals.player)
	end)
	...
	TRP3_ProfileReportButton:SetScript("OnClick", function()
		...
		Ellyb.Popups:OpenURL(
			"https://battle.net/support/help/product/wow/197/1501/solution",
			reportText,
			nil,
			loc.COPY_SYSTEM_MESSAGE
		)
	end)
