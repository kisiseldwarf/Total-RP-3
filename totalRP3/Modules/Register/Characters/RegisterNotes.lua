...
function TRP3_API.register.inits.notesInit()
	...
	setTooltipForSameFrame(
		TRP3_RegisterNotesViewProfileHelp,
		"LEFT",
		0,
		10,
		loc.REG_PLAYER_NOTES_PROFILE_NONAME,
		loc.REG_PLAYER_NOTES_PROFILE_HELP
	)
	setTooltipForSameFrame(
		TRP3_RegisterNotesViewAccountHelp,
		"LEFT",
		0,
		10,
		loc.REG_PLAYER_NOTES_ACCOUNT,
		loc.REG_PLAYER_NOTES_ACCOUNT_HELP
	)
		...
		TRP3_API.target.registerButton({
			...
			condition = function(_, unitID)
				return (
					...
					and getPlayerCurrentProfileID() ~= getConfigValue("default_profile_id")
				) or (isUnitIDKnown(unitID) and hasProfile(unitID))
			end,
