...
local get, getPlayerCurrentProfile, hasProfile =
	TRP3_API.profile.getData, TRP3_API.profile.getPlayerCurrentProfile, TRP3_API.register.hasProfile
...
local function getRelationTooltipText(profileID, profile)
	return loc:GetText("REG_RELATION_" .. getRelation(profileID) .. "_TT")
		:format(getPlayerCompleteName(true), getCompleteName(profile.characteristics or EMPTY, UNKNOWN, true))
end
...
function TRP3_API.register.unignoreID(unitID)
	...
	Events.fireEvent(
		...
		unitID,
		TRP3_API.register.isUnitIDKnown(unitID) and hasProfile(unitID) or nil,
		nil
	)
end
		...
		TRP3_API.target.registerButton({
			...
			adapter = function(buttonStructure, unitID)
				...
				buttonStructure.tooltipSub = "|cff00ff00"
					.. getRelationTooltipText(profileID, getProfile(profileID))
					.. "\n"
