...
local getConfigValue, registerConfigKey, registerHandler =
	TRP3_API.configuration.getValue, TRP3_API.configuration.registerConfigKey, TRP3_API.configuration.registerHandler
...
local function setToolbarTextFrameScript(toolbar, frame)
	frame:SetScript("OnEditFocusGained", function()
		TRP3_API.ui.text.changeToolbarTextFrame(toolbar, frame)
	end)
end
...
local function selectMusic(music)
	if music then
		TRP3_RegisterAbout_Edit_Music_Text:SetText(
			("%s: |cff00ff00%s"):format(loc.REG_PLAYER_ABOUT_MUSIC_THEME, Utils.music.getTitle(music))
		)
	else
		TRP3_RegisterAbout_Edit_Music_Text:SetText(
			("%s: |cff00ff00%s"):format(loc.REG_PLAYER_ABOUT_MUSIC_THEME, loc.REG_PLAYER_ABOUT_NOMUSIC)
		)
	end
end
...
local function showTemplate2(dataTab)
	...
	for _, frameTab in pairs(templateData) do
		...
		if frame == nil then
			frame = CreateFrame(
				"Frame",
				"TRP3_RegisterAbout_Template2_Frame" .. frameIndex,
				TRP3_RegisterAbout_AboutPanel_Template2,
				"TRP3_RegisterAbout_Template2_Frame"
			)
...
local function createTemplate2Frame(frameIndex)
	local frame = CreateFrame(
		"Frame",
		"TRP3_RegisterAbout_Template2_Edit" .. frameIndex,
		TRP3_RegisterAbout_Edit_Template2_Container,
		"TRP3_RegisterAbout_Template2_Edit"
	)
	setupListBox(
		_G["TRP3_RegisterAbout_Template2_Edit" .. frameIndex .. "Bkg"],
		getTiledBackgroundList(),
		setTemplate2EditBkg,
		nil,
		120,
		true
	...
	setToolbarTextFrameScript(
		TRP3_RegisterAbout_Edit_Toolbar,
		_G["TRP3_RegisterAbout_Template2_Edit" .. frameIndex .. "TextScrollText"]
	)
...
local function refreshConsultDisplay(context)
	...
	assert(
		type(templatesFunction[template]) == "function",
		"Error: no function for about template: " .. tostring(template)
	)
...
local function setEditTemplate(value)
	...
	if value == 1 then
		TRP3_API.ui.text.changeToolbarTextFrame(
			TRP3_RegisterAbout_Edit_Toolbar,
			TRP3_RegisterAbout_Edit_Template1ScrollText
		)
	elseif value == 2 then
		TRP3_API.ui.text.changeToolbarTextFrame(
			TRP3_RegisterAbout_Edit_Toolbar,
			TRP3_RegisterAbout_Template2_Edit1TextScrollText
		)
	else
		TRP3_API.ui.text.changeToolbarTextFrame(
			TRP3_RegisterAbout_Edit_Toolbar,
			TRP3_RegisterAbout_Edit_Template3_PhysTextScrollText
		)
	end
...
local function onAboutReceived(profileID)
	...
	local noDescr = (aboutData.TE == 1 and not shouldShowTemplate1(aboutData))
		or (aboutData.TE == 2 and not shouldShowTemplate2(aboutData))
		or (aboutData.TE == 3 and not shouldShowTemplate3(aboutData))
	if noDescr then
		aboutData.read = true
		Events.fireEvent(Events.REGISTER_ABOUT_READ)
	end
...
function TRP3_API.register.inits.aboutInit()
	...
	setupListBox(
		TRP3_RegisterAbout_Edit_TemplateField,
		{ { "Template 1", 1 }, { "Template 2", 2 }, { "Template 3", 3 } },
		setEditTemplate,
		nil,
		150,
		true
	...
	TRP3_RegisterAbout_Edit_Template3_PhysIcon:SetScript("OnClick", function()
		showIconBrowser(onPhisIconSelected)
	end)
	TRP3_RegisterAbout_Edit_Template3_PsyIcon:SetScript("OnClick", function()
		showIconBrowser(onPsychoIconSelected)
	end)
	TRP3_RegisterAbout_Edit_Template3_HistIcon:SetScript("OnClick", function()
		showIconBrowser(onHistoIconSelected)
	end)
