...
local assert, type, wipe, strconcat, pairs, tinsert, tremove, _G, strtrim =
	assert, type, wipe, strconcat, pairs, tinsert, tremove, _G, strtrim
...
local getRelationText, getRelationTooltipText, setRelation =
	RELATIONS.getRelationText, RELATIONS.getRelationTooltipText, RELATIONS.setRelation
...
local function setConsultDisplay(context)
	...
	for frameIndex, charName in pairs(shownCharacteristics) do
		...
		if frame == nil then
			frame = CreateFrame(
				"Frame",
				"TRP3_RegisterCharact_RegisterInfoLine" .. frameIndex,
				TRP3_RegisterCharact_CharactPanel_Container,
				"TRP3_RegisterCharact_RegisterInfoLine"
			)
		...
		if charName == "RE" and dataTab.RC and #dataTab.RC >= 4 then
			...
			setTooltipForSameFrame(
				TRP3_RegisterCharact_CharactPanel_ResidenceButton,
				"RIGHT",
				0,
				5,
				loc.REG_PLAYER_RESIDENCE_SHOW,
				loc.REG_PLAYER_RESIDENCE_SHOW_TT:format(dataTab.RC[4])
			)
			TRP3_RegisterCharact_CharactPanel_ResidenceButton:SetScript("OnClick", function()
				...
				AddOn_TotalRP3.Map.placeSingleMarker(
					dataTab.RC[2],
					dataTab.RC[3],
					{ characterID = characterID, profileID = profileID },
					TRP3_PlayerHousePinMixin.TEMPLATE_NAME
				)
			end)
			...
			setTooltipForSameFrame(
				...
				5,
				loc.REG_PLAYER_RESIDENCE_SHOW,
				Ellyb.ColorManager.GREEN(dataTab.RC[4])
			)
		end
		frame:Show()
		previous = frame
	...
	if type(dataTab.MI) == "table" and #dataTab.MI > 0 then
		...
		for frameIndex, miscStructure in ipairs(dataTab.MI) do
			...
			if frame == nil then
				frame = CreateFrame(
					"Frame",
					"TRP3_RegisterCharact_MiscInfoLine" .. frameIndex,
					TRP3_RegisterCharact_CharactPanel_Container,
					"TRP3_RegisterCharact_RegisterInfoLine"
				)
				scaleField(frame, TRP3_RegisterCharact_CharactPanel_Container:GetWidth())
	...
	if type(dataTab.PS) == "table" and #dataTab.PS > 0 then
		...
		for frameIndex, psychoStructure in ipairs(dataTab.PS) do
			...
			if frame == nil then
				frame = CreateFrame(
					"Frame",
					"TRP3_RegisterCharact_PsychoInfoLine" .. frameIndex,
					TRP3_RegisterCharact_CharactPanel_Container,
					"TRP3_RegisterCharact_PsychoInfoDisplayLine"
				)
...
function setEditDisplay()
	...
	TRP3_RegisterCharact_Dropdown_RelationshipField:SetSelectedValue(
		draftData.RS or AddOn_TotalRP3.Enums.RELATIONSHIP_STATUS.UNKNOWN
	)
	...
	for frameIndex, miscStructure in pairs(draftData.MI) do
		...
		if frame == nil then
			frame = CreateFrame(
				"Frame",
				"TRP3_RegisterCharact_MiscEditLine" .. frameIndex,
				TRP3_RegisterCharact_Edit_CharactPanel_Container,
				"TRP3_RegisterCharact_MiscEditLine"
			)
	...
	for _, frame in pairs(psychoEditCharFrame) do
		frame:Hide()
	end
	for frameIndex, psychoStructure in pairs(draftData.PS) do
		...
		if frame == nil then
			...
			frame = CreateFrame(
				"Frame",
				"TRP3_RegisterCharact_PsychoEditLine" .. frameIndex,
				TRP3_RegisterCharact_Edit_CharactPanel_Container,
				"TRP3_RegisterCharact_PsychoInfoEditLine"
			)
			...
			setTooltipForSameFrame(
				frame.CustomLeftIcon,
				"TOP",
				0,
				5,
				loc.UI_ICON_SELECT,
				loc.REG_PLAYER_PSYCHO_LEFTICON_TT
			)
			setTooltipForSameFrame(
				frame.CustomRightIcon,
				"TOP",
				0,
				5,
				loc.UI_ICON_SELECT,
				loc.REG_PLAYER_PSYCHO_RIGHTICON_TT
			)
			setTooltipForSameFrame(frame.DeleteButton, "TOP", 0, 5, loc.CM_REMOVE)
			setTooltipForSameFrame(
				frame.CustomLeftColor,
				"TOP",
				0,
				5,
				loc.REG_PLAYER_PSYCHO_CUSTOMCOLOR,
				loc.REG_PLAYER_PSYCHO_CUSTOMCOLOR_LEFT_TT
			)
			setTooltipForSameFrame(
				frame.CustomRightColor,
				"TOP",
				0,
				5,
				loc.REG_PLAYER_PSYCHO_CUSTOMCOLOR,
				loc.REG_PLAYER_PSYCHO_CUSTOMCOLOR_RIGHT_TT
			)
...
local function setupRelationButton(profileID, profile)
	...
	setTooltipAll(
		TRP3_RegisterCharact_ActionButton,
		"LEFT",
		0,
		0,
		loc.CM_ACTIONS,
		loc.REG_RELATION_BUTTON_TT:format(getRelationText(profileID), getRelationTooltipText(profileID, profile))
	)
end
...
local function refreshDisplay()
	...
	for _, frame in pairs(psychoCharFrame) do
		frame:Hide()
	end
...
local function onActionSelected(value)
	...
	if value == 1 then
		...
		showConfirmPopup(
			loc.REG_DELETE_WARNING:format(
				Utils.str.color("g") .. getCompleteName(profil.characteristics or {}, UNKNOWN, true) .. "|r"
			),
			function()
	...
	elseif value == 2 then
		showTextInputPopup(
			loc.REG_PLAYER_IGNORE_WARNING:format(strjoin("\n", unpack(getKeys(context.profile.link)))),
			function(text)
...
function TRP3_API.register.inits.characteristicsInit()
	...
	TRP3_RegisterCharact_Edit_NamePanel_Icon:SetScript("OnClick", function()
		showIconBrowser(onPlayerIconSelected)
	end)
	...
	-- Localz
	setTooltipForSameFrame(
		TRP3_RegisterCharact_Edit_NamePanel_Icon,
		"RIGHT",
		0,
		5,
		loc.REG_PLAYER_ICON,
		loc.REG_PLAYER_ICON_TT
	)
	setTooltipForSameFrame(
		TRP3_RegisterCharact_Edit_TitleFieldHelp,
		"RIGHT",
		0,
		5,
		loc.REG_PLAYER_TITLE,
		loc.REG_PLAYER_TITLE_TT
	)
	setTooltipForSameFrame(
		TRP3_RegisterCharact_Edit_FirstFieldHelp,
		"RIGHT",
		0,
		5,
		loc.REG_PLAYER_FIRSTNAME,
		loc.REG_PLAYER_FIRSTNAME_TT:format(Globals.player)
	)
	setTooltipForSameFrame(
		TRP3_RegisterCharact_Edit_LastFieldHelp,
		"RIGHT",
		0,
		5,
		loc.REG_PLAYER_LASTNAME,
		loc.REG_PLAYER_LASTNAME_TT
	)
	setTooltipForSameFrame(
		TRP3_RegisterCharact_Edit_FullTitleFieldHelp,
		"RIGHT",
		0,
		5,
		loc.REG_PLAYER_FULLTITLE,
		loc.REG_PLAYER_FULLTITLE_TT
	)
	setTooltipForSameFrame(
		TRP3_RegisterCharact_Edit_RaceFieldHelp,
		"RIGHT",
		0,
		5,
		loc.REG_PLAYER_RACE,
		loc.REG_PLAYER_RACE_TT:format(Globals.player_race_loc)
	)
	setTooltipForSameFrame(
		TRP3_RegisterCharact_Edit_ClassFieldHelp,
		"RIGHT",
		0,
		5,
		loc.REG_PLAYER_CLASS,
		loc.REG_PLAYER_CLASS_TT:format(Globals.player_class_loc)
	)
	setTooltipForSameFrame(
		TRP3_RegisterCharact_Edit_AgeFieldHelp,
		"RIGHT",
		0,
		5,
		loc.REG_PLAYER_AGE,
		loc.REG_PLAYER_AGE_TT
	)
	setTooltipForSameFrame(
		TRP3_RegisterCharact_Edit_BirthplaceFieldHelp,
		"RIGHT",
		0,
		5,
		loc.REG_PLAYER_BIRTHPLACE,
		loc.REG_PLAYER_BIRTHPLACE_TT
	)
	setTooltipForSameFrame(
		TRP3_RegisterCharact_Edit_ResidenceFieldHelp,
		"RIGHT",
		0,
		5,
		loc.REG_PLAYER_RESIDENCE,
		loc.REG_PLAYER_RESIDENCE_TT
	)
	setTooltipForSameFrame(
		TRP3_RegisterCharact_Edit_EyeFieldHelp,
		"RIGHT",
		0,
		5,
		loc.REG_PLAYER_EYE,
		loc.REG_PLAYER_EYE_TT
	)
	setTooltipForSameFrame(
		TRP3_RegisterCharact_Edit_HeightFieldHelp,
		"RIGHT",
		0,
		5,
		loc.REG_PLAYER_HEIGHT,
		loc.REG_PLAYER_HEIGHT_TT
	)
	setTooltipForSameFrame(
		TRP3_RegisterCharact_Edit_WeightFieldHelp,
		"RIGHT",
		0,
		5,
		loc.REG_PLAYER_WEIGHT,
		loc.REG_PLAYER_WEIGHT_TT
	)
	setTooltipForSameFrame(
		TRP3_RegisterCharact_Edit_BirthplaceButton,
		"RIGHT",
		0,
		5,
		loc.REG_PLAYER_HERE,
		loc.REG_PLAYER_HERE_TT
	)
	setTooltipForSameFrame(
		TRP3_RegisterCharact_Edit_EyeButton,
		"RIGHT",
		0,
		5,
		loc.REG_PLAYER_EYE,
		loc.REG_PLAYER_COLOR_TT
	)
	setTooltipForSameFrame(
		TRP3_RegisterCharact_Edit_ClassButton,
		"RIGHT",
		0,
		5,
		loc.REG_PLAYER_COLOR_CLASS,
		loc.REG_PLAYER_COLOR_CLASS_TT .. loc.REG_PLAYER_COLOR_TT
	)
	...
	TRP3_RegisterCharact_CharactPanel_RegisterTitle:SetText(
		Utils.str.icon(TRP3_InterfaceIcons.DirectorySection, 25) .. " " .. loc.REG_PLAYER_REGISTER
	)
	TRP3_RegisterCharact_CharactPanel_Edit_RegisterTitle:SetText(
		Utils.str.icon(TRP3_InterfaceIcons.DirectorySection, 25) .. " " .. loc.REG_PLAYER_REGISTER
	)
	TRP3_RegisterCharact_CharactPanel_PsychoTitle:SetText(
		Utils.str.icon(TRP3_InterfaceIcons.TraitSection, 25) .. " " .. loc.REG_PLAYER_PSYCHO
	)
	TRP3_RegisterCharact_CharactPanel_Edit_PsychoTitle:SetText(
		Utils.str.icon(TRP3_InterfaceIcons.TraitSection, 25) .. " " .. loc.REG_PLAYER_PSYCHO
	)
	TRP3_RegisterCharact_CharactPanel_MiscTitle:SetText(
		Utils.str.icon(TRP3_InterfaceIcons.MiscInfoSection, 25) .. " " .. loc.REG_PLAYER_MORE_INFO
	)
	TRP3_RegisterCharact_CharactPanel_Edit_MiscTitle:SetText(
		Utils.str.icon(TRP3_InterfaceIcons.MiscInfoSection, 25) .. " " .. loc.REG_PLAYER_MORE_INFO
	)
	...
	setupListBox(
		TRP3_RegisterCharact_Dropdown_RelationshipField,
		RELATIONSHIP_STATUS_DROPDOWN,
		onRelationshipStatusSelection,
		loc.REG_PLAYER_RELATIONSHIP_STATUS_UNKNOWN,
		200,
		false
	)
	Ellyb.Tooltips
		.getTooltip(TRP3_RegisterCharact_Dropdown_RelationshipField)
