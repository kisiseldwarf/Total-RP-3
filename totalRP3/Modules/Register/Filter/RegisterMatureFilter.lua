...
local function onStart()
	-- TRP3 API imports
	local Utils, Events, Register, UI, Config =
		TRP3_API.utils, TRP3_API.events, TRP3_API.register, TRP3_API.ui, TRP3_API.configuration
	...
	local function addWordsToCustomDictionary(words)
		...
		assert(
			type(words) == "table" or type(words) == "string",
			"Trying to call addWordsToCustomDictionary with a value that is neither a table nor a string."
		)
	...
	local function removeWordFromCustomDictionary(word)
		...
		assert(
			TRP3_MatureFilter.dictionary[word],
			("Trying to remove unkown word %s from custom dictionary."):format(word)
		)
	...
	for line = 0, 8 do
		local lineFrame = CreateFrame(
			"Button",
			"TRP3_MatureDictionaryEditorButton_" .. line,
			dictionaryEditor.content,
			"TRP3_MatureDictionaryLine"
		)
	...
	matureFilterShouldBeEnabledByDefault = C_StorePublic.IsDisabledByParentalControls()
		or matureFilterShouldBeEnabledByDefault
	...
	local function shouldReEvaluateContent(lastMatureContentEvaluation)
		if not lastMatureContentEvaluation then
			return true
		end
		local now = time()
	...
	Events.listenToEvent(Events.REGISTER_DATA_UPDATED, function(_, profileID, _)
		if
			isMatureFilterEnabled()
			and profileID
			and getProfileOrNil(profileID)
			and not isProfileSafeListed(profileID)
		then
			local profile = getProfileByID(profileID)
	...
	TRP3_API.register.mature_filter.whitelistProfileID = Ellyb.DeprecationWarnings.wrapFunction(
		addProfileIdToSafeList,
		"TRP3_API.register.mature_filter.whitelistProfileID",
		"TRP3_API.register.mature_filter.addProfileIdToSafeList"
	)
	TRP3_API.register.mature_filter.whitelistProfileByUnitIDConfirm = Ellyb.DeprecationWarnings.wrapFunction(
		presentAddUnitToSafeListPopup,
		"TRP3_API.register.mature_filter.whitelistProfileByUnitIDConfirm",
		"TRP3_API.register.mature_filter.presentAddUnitToSafeListPopup"
	)
	TRP3_API.register.mature_filter.isProfileWhitelisted = Ellyb.DeprecationWarnings.wrapFunction(
		isProfileSafeListed,
		"TRP3_API.register.mature_filter.isProfileWhitelisted",
		"TRP3_API.register.mature_filter.isProfileSafeListed"
	)
end
