...
local function onStart()
	...
	local function updateMiscInfoField(profile, field, value)
		...
		local miscIndex = FindInTableIf(miscInfo, function(miscStruct)
			return miscStruct.NA == fieldInfo.localizedText or miscStruct.NA == fieldInfo.englishText
		end)
				...
				then
					...
					if CHARACTERISTICS_FIELDS[field] then
						...
						if field == "PS" and value then
							profile.characteristics[CHARACTERISTICS_FIELDS[field]] =
								AddOn_TotalRP3.MSP.DeserializeField(field, value)
						end
