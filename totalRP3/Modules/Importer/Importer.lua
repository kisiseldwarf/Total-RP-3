...
TRP3_API.importer.addAddOn = function(addOnName, API)
	if
		type(API.isAvailable) == "function"
		and type(API.addOnVersion) == "function"
		and type(API.getProfile) == "function"
		and type(API.getFormatedProfile) == "function"
		and type(API.listAvailableProfiles) == "function"
		and type(API.getImportableData) == "function"
	then
		log("Importer : API registered " .. addOnName)
		addOns[addOnName] = API
	else
		print(
			"An API for the addon "
				.. addOnName
				.. " tried to register itself in the importer module, but misses some of the required functions."
		)
	end
end
