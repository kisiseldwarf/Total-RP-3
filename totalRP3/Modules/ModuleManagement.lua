...
local pairs, type, assert, pcall, tinsert, table, _G, tostring =
	pairs, type, assert, pcall, tinsert, table, _G, tostring
local Log = Utils.log
...
local setTooltipForSameFrame, setTooltipAll =
	TRP3_API.ui.tooltip.setTooltipForSameFrame, TRP3_API.ui.tooltip.setTooltipAll
...
TRP3_API.module.registerModule = function(moduleStructure)
	...
	assert(
		not hasBeenInit,
		"Module structure must be registered before Total RP 3 initialization: " .. moduleStructure.id
	)
...
local function checkModuleDependency(_, dependency_id, dependency_version)
	return MODULE_REGISTRATION[dependency_id]
		and MODULE_REGISTRATION[dependency_id].version >= dependency_version
		and MODULE_ACTIVATION[dependency_id] ~= false
end
...
local function handleModuleError(module, err)
	...
	DEFAULT_CHAT_FRAME:AddMessage(
		('|cffff0000[TotalRP3] Error while loading module "%s": |r%s'):format(tostring(module.id), tostring(err)),
		1,
		1,
		1
	)
...
function onModuleStarted()
	...
	for _, moduleID in pairs(sortedID) do
		...
		local frame = CreateFrame(
			"Frame",
			"TRP3_ConfigurationModule_" .. i,
			TRP3_ConfigurationModuleContainer,
		...
		setTooltipForSameFrame(
			_G[frame:GetName() .. "Info"],
			"BOTTOMRIGHT",
			0,
			0,
			module.name,
			getModuleTooltip(module)
		)
		if module.status == MODULE_STATUS.OK then
