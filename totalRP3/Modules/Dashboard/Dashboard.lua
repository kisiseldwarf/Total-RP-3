...
TRP3_API.events.listenToEvent(TRP3_API.events.WORKFLOW_ON_LOADED, function()
	...
	if TRP3_API.toolbar then
		...
		local status1SubText = color("y")
			.. loc.CM_CLICK
			.. ": "
			.. color("w")
			.. (loc.TB_GO_TO_MODE:format(color("g") .. loc.TB_NORMAL_MODE .. color("w")))
		local status2Text = color("w") .. loc.TB_STATUS .. ": " .. color("o") .. loc.TB_AFK_MODE
		local status2SubText = color("y")
			.. loc.CM_CLICK
			.. ": "
			.. color("w")
			.. (loc.TB_GO_TO_MODE:format(color("g") .. loc.TB_NORMAL_MODE .. color("w")))
		local status3Text = color("w") .. loc.TB_STATUS .. ": " .. color("g") .. loc.TB_NORMAL_MODE
		local status3SubText = color("y")
			.. loc.CM_L_CLICK
			.. ": "
			.. color("w")
			.. (loc.TB_GO_TO_MODE:format(color("o") .. loc.TB_AFK_MODE .. color("w")))
			.. "\n"
			.. color("y")
			.. loc.CM_R_CLICK
			.. ": "
			.. color("w")
			.. (loc.TB_GO_TO_MODE:format(color("r") .. loc.TB_DND_MODE .. color("w")))
		local Button_Status = {
			id = "aa_trp3_c",
					...
					for key, value in pairs(list) do
						...
						if key == currentProfileID then
							tinsert(dropdownItems, {
								"|Tinterface\\icons\\" .. icon .. ":15|t|cff00ff00 " .. value.profileName .. "|r",
								nil,
							})
						else
							tinsert(
								dropdownItems,
								{ "|Tinterface\\icons\\" .. icon .. ":15|t " .. value.profileName, key }
							)
						end
					end
			...
			local Button_Helmet = {
				...
				onModelUpdate = function(buttonStructure)
					if ShowingHelm() then
						...
						local currentHelmetTexture =
							GetInventoryItemTexture("player", select(1, GetInventorySlotInfo("HeadSlot")))
			...
			local Button_Cape = {
				...
				onModelUpdate = function(buttonStructure)
					if ShowingCloak() then
						...
						local currentCloakTexture =
							GetInventoryItemTexture("player", select(1, GetInventorySlotInfo("BackSlot")))
