...
TRP3_API.events.listenToEvent(TRP3_API.events.WORKFLOW_ON_LOADED, function()
	...
	local function constructMessage(NPCName, channel, message)
		return strconcat(
			TRP3_API.chat.configNPCTalkPrefix(),
			NPCName or "",
			" ",
			getChannelActionString(channel),
			message or ""
		)
	end
	...
	local function sendNPCTalk()
		...
		if not message or strlen(strtrim(message)) < 1 then
			return
		end
	...
	local function buildChannelDropdown()
		...
		for channelLabel, channelName in pairs(CHANNEL_TYPES) do
			...
			tinsert(channelTypes, {
				TRP3_API.Ellyb.ColorManager.YELLOW(loc.NPC_TALK_CHANNEL) .. channelColor(_G[channelLabel]),
				channelName,
			})
		end
