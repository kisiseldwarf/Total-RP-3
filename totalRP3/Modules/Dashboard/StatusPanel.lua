...
function DashboardStatusPanelMixin:OnWorkflowLoaded()
	...
	Config.registerHandler({ "AddonLocale" }, function(...)
...
function DashboardStatusMenuMixin:OnLoad()
	...
	Events.registerCallback(Events.REGISTER_DATA_UPDATED, function(...)
...
-- our own.
--[[protected]]
function DashboardStatusMenuMixin:CreateMenuItem()
...
--[[protected]]
function DashboardStatusMenuMixin:AddButtonToMenu(item, level)
	-- Preconditions: We must be initializing our own menu.
	assert(MSA_DROPDOWNMENU_INIT_MENU == self)
...
--[[override]]
function DashboardRPStatusMenuMixin:GetMenuItemText(itemValue)
	local itemText
...
--[[override]]
function DashboardRPStatusMenuMixin:GetSelectedMenuItem()
	local currentUser = AddOn_TotalRP3.Player.GetCurrentUser()
	return currentUser:GetRoleplayStatus()
end

--[[override]]
function DashboardRPStatusMenuMixin:OnMenuInitialize(level)
...
--[[override]]
function DashboardRPStatusMenuMixin:OnMenuButtonClicked(button)
...
--[[override]]
function DashboardXPStatusMenuMixin:GetMenuItemText(itemValue)
...
--[[override]]
function DashboardXPStatusMenuMixin:GetSelectedMenuItem()
	local currentUser = AddOn_TotalRP3.Player.GetCurrentUser()
	return currentUser:GetRoleplayExperience()
end

--[[override]]
function DashboardXPStatusMenuMixin:OnMenuInitialize(level)
	DashboardStatusMenuMixin.OnMenuInitialize(self, level)
...
--[[override]]
function DashboardXPStatusMenuMixin:OnMenuButtonClicked(button)
	DashboardStatusMenuMixin.OnMenuButtonClicked(self, button)
