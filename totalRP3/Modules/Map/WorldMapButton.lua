...
Events.registerCallback(Events.WORKFLOW_ON_LOADED, function()
	...
	local function placeMapButton(newPosition)
		if
			getConfigValue(CONFIG_HIDE_BUTTON_IF_EMPTY)
			and Ellyb.Tables.isEmpty(TRP3_API.MapScannersManager.getAllScans())
		then
			WorldMapButton:Hide()
			return
		else
		...
		if newPosition then
			setConfigValue(CONFIG_MAP_BUTTON_POSITION, newPosition)
		end
	...
	---@type Tooltip
	Ellyb.Tooltips.getTooltip(WorldMapButton):SetTitle(loc.MAP_BUTTON_TITLE):OnShow(function(tooltip)
