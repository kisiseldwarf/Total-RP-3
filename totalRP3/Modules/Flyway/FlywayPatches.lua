...
TRP3_API.flyway.patches["4"] = function()
	if
		TRP3_Configuration
		and TRP3_Configuration["register_mature_filter"]
		and TRP3_Configuration["register_mature_filter"] == "0"
	then
...
TRP3_API.flyway.patches["10"] = function()
	...
	if TRP3_Profiles then
		for _, profile in pairs(TRP3_Profiles) do
			if
				profile.player
				and profile.player.about
				and profile.player.about.MU
				and type(profile.player.about.MU) == "string"
			then
