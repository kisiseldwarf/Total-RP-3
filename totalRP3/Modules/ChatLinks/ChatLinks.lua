...
TRP3_API.events.listenToEvent(TRP3_API.events.WORKFLOW_ON_LOADED, function()
		...
		if linkType == LINK_CODE then
			if IsShiftKeyDown() then
				...
				if activeChatFrame and activeChatFrame.chatFrame and activeChatFrame.chatFrame.editBox then
					activeChatFrame.chatFrame.editBox:SetText(
						TRP3_API.utils.str.sanitize(activeChatFrame.chatFrame.editBox:GetText())
					)
				end
			else
	...
	AddOn_TotalRP3.Communications.registerSubSystemPrefix(
		CHAT_LINKS_PROTOCOL_REQUEST_PREFIX,
		function(identifier, sender)
	...
	function ChatLinks:OpenMakeImportablePrompt(linkType, callback)
		...
		TRP3_API.popup.showCustomYesNoPopup(
			loc.CL_MAKE_IMPORTABLE_SIMPLER:format(TRP3_API.Ellyb.ColorManager.ORANGE(linkType)),
