...
local function onStart()
	...
	local function getTooltipTitleWithIcon(buttonStructure)
		return Utils.str.icon(buttonStructure.icon, 25)
			.. " "
			.. (buttonStructure.tooltip or buttonStructure.configText)
	end
			...
			for _, id in pairs(ids) do
				...
				if uiButton == nil then -- Create the button
					uiButton = CreateFrame(
						"Button",
						"TRP3_ToolbarButton" .. index,
						toolbarContainer,
						"TRP3_ToolbarButtonTemplate"
					)
				...
				if buttonStructure.tooltip then
					setTooltipForFrame(
						...
						0,
						0,
						getTooltipTitleWithIcon(buttonStructure),
	...
	local function registerDatabrokerButton(buttonStructure)
		local LDBObject =
			LDB:NewDataObject(TRP3_API.globals.addon_name_short .. " — " .. buttonStructure.configText, {
				type = "data source",
	...
	local function toolbarAddButton(buttonStructure)
		...
		assert(
			not buttonStructures[buttonStructure.id],
			"The toolbar button with id " .. buttonStructure.id .. " already exists."
		)
	...
	toolbar:SetPoint(
		getConfigValue("CONFIG_TOOLBAR_POS_A"),
		UIParent,
		getConfigValue("CONFIG_TOOLBAR_POS_A"),
		getConfigValue("CONFIG_TOOLBAR_POS_X"),
		getConfigValue("CONFIG_TOOLBAR_POS_Y")
	)
