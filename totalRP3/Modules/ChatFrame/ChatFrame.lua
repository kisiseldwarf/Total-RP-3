...
local getUnitIDCurrentProfile, getUnitRPName, getUnitRPFirstName, getUnitRPLastName =
	TRP3_API.register.getUnitIDCurrentProfile,
	TRP3_API.register.getUnitRPName,
	TRP3_API.register.getUnitRPFirstName,
	TRP3_API.register.getUnitRPLastName
...
---@deprecated
TRP3_API.chat.configIncreaseNameColorContrast = TRP3_API.Ellyb.DeprecationWarnings.wrapFunction(
	AddOn_TotalRP3.Configuration.shouldDisplayIncreasedColorContrast,
	"TRP3_API.chat.configIncreaseNameColorContrast",
	"AddOn_TotalRP3.Configuration.shouldDisplayIncreasedColorContrast"
)
...
TRP3_API.chat.getEmoteDetectionColor = function()
	return ColorManager.getChatColorForChannel("EMOTE")
end
TRP3_API.chat.getSpeechDetectionColor = function()
	return ColorManager.getChatColorForChannel("SAY")
end
...
local function handleNPCEmote(message, senderID)
	...
	message = message:gsub("%[.-%]", function(name)
		return wrapNameInColorForNPCEmote(name, senderID, chatColor)
	end)
...
function Utils.customGetColoredNameWithCustomFallbackFunction(
	fallback,
	event,
	arg1,
	arg2,
	...
	assert(
		fallback,
		"Trying to call TRP3_API.utils.customGetColoredNameWithCustomFallbackFunction(fallback, event, ...) without a fallback function!"
	)
	...
	if
		not isChannelHandled(event)
		or not configIsChannelUsed(event)
		or not GUID
		or not Utils.guid.isAPlayerGUID(GUID)
	then
		return fallback(event, arg1, arg2, arg3, arg4, arg5, arg6, arg7, channelNumber, arg9, arg10, arg11, arg12)
	end
...
function hooking()
	...
	hooksecurefunc("ChatEdit_InsertLink", function(unitID)
		...
		if not configInsertFullRPName() or IsAltKeyDown() then
			return
		end

		-- Do not modify the name if we don't know that character
		if not (IsUnitIDKnown(unitID) or unitID == Globals.player_id) then
			return
		end
