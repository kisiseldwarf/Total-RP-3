...
function Languages.setLanguageByID(languageID)
	...
	if not language then
		...
		Log.log(
			"Trying to set a language that is not known by this character, going back to default language: " .. language,
			Log.level.WARNING
		)
	end
...
function Languages.getSavedLanguage()
	if
		TRP3_Characters
		and TRP3_Characters[Globals.player_id]
		and TRP3_Characters[Globals.player_id][LAST_LANGUAGE_USED]
	then
...
TRP3_API.events.listenToEvent(TRP3_API.events.WORKFLOW_ON_LOADED, function()
		...
		onModelUpdate = function(buttonStructure)
			if buttonStructure.currentLanguageID ~= ChatFrame1EditBox.languageID then
				...
				buttonStructure.tooltipSub =
					Ellyb.Strings.clickInstruction(Ellyb.System.CLICKS.CLICK, loc.TB_LANGUAGES_TT)
			...
			for _, language in ipairs(Languages.getAvailableLanguages()) do
				if language:IsActive() then
					tinsert(dropdownItems, {
						language:GetIcon():GenerateString(15) .. " " .. TRP3_API.Ellyb.ColorManager.GREEN(
							language:GetName()
						),
						nil,
					})
	...
	local function checkCurrentLanguageAndRestoreSavedState()
		...
		if not savedLanguage:IsKnown() then
			...
			TRP3_API.utils.message.displayMessage(
				loc.LANG_CHANGE_CAUSED_REVERT_TO_DEFAULT:format(defaultLanguage:GetName(), savedLanguage:GetName())
			)
		else
...
-- Remember last language used
TRP3_API.ADVANCED_SETTINGS_KEYS.USE_WORKAROUND_FOR_LANGUAGE_RESET =
	"chat_language_enabled_workaround_for_language_reset"
