...
local function onStart()
	...
	local getConfigValue, registerConfigKey, registerConfigHandler, setConfigValue =
		TRP3_API.configuration.getValue,
		TRP3_API.configuration.registerConfigKey,
		TRP3_API.configuration.registerHandler,
		TRP3_API.configuration.setValue
	local assert, pairs, tinsert, table, math, _G = assert, pairs, tinsert, table, math, _G
	local getUnitID, unitIDToInfo, companionIDToInfo =
		Utils.str.getUnitID, Utils.str.unitIDToInfo, Utils.str.companionIDToInfo
	local setTooltipForSameFrame, mainTooltip, refreshTooltip =
		TRP3_API.ui.tooltip.setTooltipForSameFrame, TRP3_MainTooltip, TRP3_API.ui.tooltip.refresh
	local get = TRP3_API.profile.getData
	local setupFieldSet = TRP3_API.ui.frame.setupFieldPanel
	local originalGetTargetType, getCompanionFullID =
		TRP3_API.ui.misc.getTargetType, TRP3_API.ui.misc.getCompanionFullID
	...
	local function createButton(index)
		local uiButton =
			CreateFrame("Button", "TRP3_TargetFrameButton" .. index, ui_TargetFrame, "TRP3_TargetFrameButton")
	...
	local function displayButtonsPanel()
		...
		for id, buttonStructure in pairs(targetButtons) do
			if
				buttonStructure.visible
				and (not buttonStructure.condition or buttonStructure.condition(currentTargetType, currentTargetID))
				and (not buttonStructure.onlyForType or buttonStructure.onlyForType == currentTargetType)
			then
				tinsert(ids, id)
			end
		end
	...
	local function displayTargetName()
		if currentTargetType == TRP3_Enums.UNIT_TYPE.CHARACTER then
			...
			if info.characteristics then
				setupFieldSet(
					ui_TargetFrame,
					(info.characteristics.FN or name) .. " " .. (info.characteristics.LN or ""),
					TARGET_NAME_WIDTH
				)
			else
		...
		elseif
			currentTargetType == TRP3_Enums.UNIT_TYPE.PET or currentTargetType == TRP3_Enums.UNIT_TYPE.BATTLE_PET
		then
	...
	local function shouldShowTargetFrame(config)
		if
			currentTargetID == nil
			or (getConfigValue(config) ~= 1 and (getConfigValue(config) ~= 2 or not isPlayerIC()))
		then
			return false
		elseif
			currentTargetType == TRP3_Enums.UNIT_TYPE.CHARACTER
			and (
				currentTargetID == Globals.player_id
				or (not isIDIgnored(currentTargetID) and isUnitIDKnown(currentTargetID))
			)
		then
			return true
		elseif
	...
	local function reposition()
		ui_TargetFrame:SetPoint(
			getConfigValue("CONFIG_TARGET_POS_A"),
			UIParent,
			getConfigValue("CONFIG_TARGET_POS_A"),
			getConfigValue("CONFIG_TARGET_POS_X"),
			getConfigValue("CONFIG_TARGET_POS_Y")
		)
	end
	reposition()
	...
	Events.listenToEvent(Events.REGISTER_DATA_UPDATED, function(unitID, _, dataType)
		if
			(not unitID or (currentTargetID == unitID))
			and (not dataType or dataType == "characteristics" or dataType == "about")
		then
			onTargetChanged()
		end
	end)
