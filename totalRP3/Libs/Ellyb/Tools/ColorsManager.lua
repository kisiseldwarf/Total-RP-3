...
function ColorManager.isTextColorReadableOnADarkBackground(color)
	return (0.299 * color:GetRed() + 0.587 * color:GetGreen() + 0.114 * color:GetBlue()) >= 0.5
end
...
ColorManager.EVOKER =
	Ellyb.Color:new(RAID_CLASS_COLORS[Ellyb.Enum.CLASSES.EVOKER] or { r = 0.2, g = 0.58, b = 0.5 }):Freeze()
...
ColorManager.ITEM_POOR =
	Ellyb.Color:new(BAG_ITEM_QUALITY_COLORS[Enum.ItemQuality.Common or Enum.ItemQuality.Standard]):Freeze()
ColorManager.ITEM_COMMON = Ellyb.Color:new(0.95, 0.95, 0.95):Freeze() -- Common quality is a slightly faded white
ColorManager.ITEM_UNCOMMON =
	Ellyb.Color:new(BAG_ITEM_QUALITY_COLORS[Enum.ItemQuality.Uncommon or Enum.ItemQuality.Good]):Freeze()
ColorManager.ITEM_RARE =
	Ellyb.Color:new(BAG_ITEM_QUALITY_COLORS[Enum.ItemQuality.Rare or Enum.ItemQuality.Superior]):Freeze()
