...
function Compression.decompress(compressedData, wasReceivedViaAddOnChannel)
	...
	if decompressedData == nil then
		error(
			RED("[AddOn_TotalRP3.Compression.decompress ERROR]:")
				.. '\nCould not decompress data "'
				.. GREY(tostring(compressedData))
				.. '"'
		)
	end
	...
	Functions = {
		{
			...
			Arguments = {
				...
				{
					Name = "willBeSentViaAddOnChannel",
					Type = "boolean",
					Nilable = true,
					Documentation = {
						"Set to true if the data is meant to be sent via addon channels. Some special characters not suitable for communication channels will be escaped.",
					},
				},
			},
			Returns = {
				{
					...
					Type = "sring",
					Nilable = false,
					Documentation = { "The compressed data." },
				},
			},
		},
		{
			...
			Arguments = {
				{
					...
					Type = "string",
					Nilable = false,
					Documentation = { "The data that was compressed and that we want to decompress" },
				},
				{
					Name = "wasReceivedViaAddOnChannel",
					Type = "boolean",
					Nilable = true,
					Documentation = {
						"Set to true if the data was received via addon channels. Some special characters have been escaped and need to be unescaped before the data can be decompressed.",
					},
				},
			},
			Returns = {
