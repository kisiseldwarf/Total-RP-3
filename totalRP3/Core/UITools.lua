...
local floor, tinsert, pairs, wipe, assert, _G, tostring, table, type, strconcat =
	floor, tinsert, pairs, wipe, assert, _G, tostring, table, type, strconcat
local math = math
local MouseIsOver, CreateFrame, ToggleDropDownMenu = MouseIsOver, CreateFrame, MSA_ToggleDropDownMenu
local UIDropDownMenu_Initialize, UIDropDownMenu_CreateInfo, UIDropDownMenu_AddButton =
	MSA_DropDownMenu_Initialize, MSA_DropDownMenu_CreateInfo, MSA_DropDownMenu_AddButton
local TRP3_MainTooltip, TRP3_MainTooltipTextRight1, TRP3_MainTooltipTextLeft1, TRP3_MainTooltipTextLeft2 =
	TRP3_MainTooltip, TRP3_MainTooltipTextRight1, TRP3_MainTooltipTextLeft1, TRP3_MainTooltipTextLeft2
...
local function openDropDown(anchoredFrame, values, callback, space, addCancel)
	...
	UIDropDownMenu_Initialize(dropDownFrame, function(_, level, menuList)
...
local function setupDropDownMenu(hasClickFrame, values, callback, space, addCancel, rightClick)
	hasClickFrame:SetScript("OnClick", function(_, button)
		if (rightClick and button ~= "RightButton") or (not rightClick and button ~= "LeftButton") then
			return
		end
...
-- Setup the frame tooltip (position and text)
-- The tooltip can be shown by using refreshTooltip(Frame)
TRP3_API.ui.tooltip.setTooltipForSameFrame =
	function(Frame, GenFrameAnch, GenFrameX, GenFrameY, titleText, bodyText, rightText)
...
local findPetOwner, findBattlePetOwner, UnitName =
	TRP3_API.Locale.findPetOwner, TRP3_API.Locale.findBattlePetOwner, UnitName
...
local function insertContainerTag(alignIndex, button, frame)
	...
	insertTag(
		strconcat(tagInfo.openTags[alignIndex], loc.REG_PLAYER_ABOUT_T1_YOURTEXT, tagInfo.closeTag),
		cursorIndex,
		frame
	)
...
local function onContainerTagClicked(button, frame, isP)
	...
	openDropDown(button, values, function(alignIndex, mouseButton)
...
function TRP3_API.ui.text.setupToolbar(toolbar, textFrame, parentFrame, point, parentPoint)
	...
	toolbar.h1:SetScript("OnClick", function(button)
		if toolbar.textFrame then
			onContainerTagClicked(button, toolbar.textFrame)
		end
	end)
	toolbar.h2:SetScript("OnClick", function(button)
		if toolbar.textFrame then
			onContainerTagClicked(button, toolbar.textFrame)
		end
	end)
	toolbar.h3:SetScript("OnClick", function(button)
		if toolbar.textFrame then
			onContainerTagClicked(button, toolbar.textFrame)
		end
	end)
	toolbar.p:SetScript("OnClick", function(button)
		if toolbar.textFrame then
			onContainerTagClicked(button, toolbar.textFrame, true)
		end
	end)
	toolbar.icon:SetScript("OnClick", function()
		if toolbar.textFrame then
			TRP3_API.popup.showPopup(
				...
				{
					function(icon)
						onIconTagSelected(icon, toolbar.textFrame)
					end,
				}
			)
		end
	...
	toolbar.image:SetScript("OnClick", function()
		if toolbar.textFrame then
			TRP3_API.popup.showPopup(
				...
				{
					function(image)
						onImageTagSelected(image, toolbar.textFrame)
					end,
				}
			)
		end
	end)
	toolbar.link:SetScript("OnClick", function()
		if toolbar.textFrame then
			onLinkTagClicked(toolbar.textFrame)
		end
	end)
end
