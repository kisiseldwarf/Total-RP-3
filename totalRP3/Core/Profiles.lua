...
local strsplit, tinsert, pairs, type, assert, _G, table, tostring, error, wipe =
	strsplit, tinsert, pairs, type, assert, _G, table, tostring, error, wipe
...
local function selectProfile(profileID)
	...
	displayMessage(
		loc.PR_PROFILE_LOADED:format(Utils.str.color("g") .. profiles[character.profileID]["profileName"] .. "|r")
	)
...
local function uiInitProfileList()
	...
	for profileID, _ in pairs(profiles) do
		if
			...
			and (
				not profileSearch
				or string.find(profiles[profileID].profileName:lower(), profileSearch:lower(), 1, true)
			)
		then
...
local function uiCheckNameAvailability(profileName)
	if not isProfileNameAvailable(profileName) then
		TRP3_API.ui.tooltip.toast(
			loc.PR_PROFILEMANAGER_ALREADY_IN_USE:format(Utils.str.color("r") .. profileName .. "|r"),
			3
		)
		return false
	end
...
local function uiDeleteProfile(profileID)
	showConfirmPopup(
		loc.PR_PROFILEMANAGER_DELETE_WARNING:format(Utils.str.color("g") .. profiles[profileID].profileName .. "|r"),
		function()
...
local function onActionSelected(value, button)
	...
	elseif value == 4 then
		...
		if serial:len() < 20000 then
			...
			TRP3_ProfileExport.content.title:SetText(
				loc.PR_EXPORT_NAME:format(profile.profileName, serial:len() / 1024)
			)
...
function TRP3_API.profile.init()
	...
	setTooltipForSameFrame(
		TRP3_ProfileManagerInfo,
		"RIGHT",
		0,
		0,
		loc.PR_EXPORT_IMPORT_TITLE,
		loc.PR_EXPORT_IMPORT_HELP
	)
	...
	-- Export/Import
	local exportWarningText = TRP3_API.Ellyb.System.IsMac() and loc.PR_EXPORT_WARNING_MAC
		or loc.PR_EXPORT_WARNING_WINDOWS
	TRP3_ProfileExport.title:SetText(loc.PR_EXPORT_PROFILE)
	TRP3_ProfileExport.warning:SetText(
		TRP3_API.Ellyb.ColorManager.RED(loc.PR_EXPORT_WARNING_TITLE) .. "\n" .. exportWarningText
	)
		...
		if object and type(object) == "table" and #object == 3 then
			...
			local import = function()
				...
				if data then
					...
					if
						data.player
						and data.player.about
						and data.player.about.MU
						and type(data.player.about.MU) == "string"
					then
			...
			if version ~= Globals.version then
				showConfirmPopup(
					loc.PR_PROFILEMANAGER_IMPORT_WARNING_2:format(
						Utils.str.color("g") .. profiles[profileID].profileName .. "|r"
					),
					import
				)
			else
				showConfirmPopup(
					loc.PR_PROFILEMANAGER_IMPORT_WARNING:format(
						Utils.str.color("g") .. profiles[profileID].profileName .. "|r"
					),
					import
				)
