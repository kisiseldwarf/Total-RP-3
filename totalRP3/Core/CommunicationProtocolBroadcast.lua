...
local wipe, string, pairs, strsplit, assert, tinsert, type, tostring =
	wipe, string, pairs, strsplit, assert, tinsert, type, tostring
...
local function broadcast(command, ...)
	if
		TRP3_ClientFeatures.BroadcastMethod == TRP3_BroadcastMethod.Channel and not config_UseBroadcast()
		or not command
	then
		Log.log("Bad params")
		return
	...
	if
		TRP3_ClientFeatures.BroadcastMethod == TRP3_BroadcastMethod.Channel
		and not helloWorlded
		and command ~= HELLO_CMD
	then
...
local function sendP2PMessage(target, command, ...)
	...
	if message:len() < 254 then
		...
		Log.log(
			("Trying a P2P message with a message with lenght %s. Abord !"):format(message:len()),
			Log.level.WARNING
		)
	end
end
...
local function onMessageReceived(...)
	...
	if prefix == BROADCAST_HEADER then
		...
		if not isIDIgnored(sender) then
			...
			if
				distributionType == "YELL"
				or distributionType == "UNKNOWN"
				or distributionType == "CHANNEL"
					and string.lower(channel) == string.lower(config_BroadcastChannel())
			then
...
local function hideBroadcastChannelFromChatFrame()
	...
	if not chatFrame then
		return
	end -- In some instances we cannot get a chat frame (Details!?!?)
...
local function moveBroadcastChannelToTheBottomOfTheList(forceMove)
	if
		getConfigValue(TRP3_API.ADVANCED_SETTINGS_KEYS.MAKE_SURE_BROADCAST_CHANNEL_IS_LAST)
		and (forceMove or helloWorlded)
	then
		local broadcastChannelIndex = GetChannelName(config_BroadcastChannel())
		if broadcastChannelIndex == nil then
			return
		end
...
Comm.broadcast.init = function()
		...
		if config_UseBroadcast() then
			...
			ticker = C_Timer.NewTicker(1, function(_)
				...
				if GetChannelName(string.lower(config_BroadcastChannel())) == 0 then
					...
					Log.log(
						"Step 2: Connected to broadcast channel: "
							.. config_BroadcastChannel()
							.. ". Now sending HELLO command."
					)
					...
					if not helloWorlded then
						broadcast(
							...
							Globals.extended_version,
							Utils.str.sanitizeVersion(Globals.extended_display_version)
						)
					end
				end
			end, 15)
	...
	if TRP3_ClientFeatures.BroadcastMethod == TRP3_BroadcastMethod.Channel then
		...
		Utils.event.registerHandler("CHAT_MSG_CHANNEL_NOTICE_USER", function(mode, user, _, _, _, _, _, _, channel)
			if
				mode == "OWNER_CHANGED"
				and user == TRP3_API.globals.player_id
				and channel == config_BroadcastChannel()
			then
