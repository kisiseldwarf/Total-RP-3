...
function Utils.color.getUnitCustomColor(unitID)
	...
	if info.characteristics and info.characteristics.CH then
		return TRP3_API.GetColorFromString(info.characteristics.CH);
	end
end
...
function TRP3_API.utils.color.GetClassColorByGUID(GUID)
	...
	return TRP3_API.GetClassDisplayColor(englishClass);
end
...
local function GenerateIndentationMarkup(data)
	local _, width = string.split(":", data);
	width = tonumber(width) or 25;

	return string.format("|T982414:1:%1$d|t", width);
end

local function GenerateHorizontalRuleMarkup(data)
	local _, width, height, color, align = string.split(":", data);

	width = tonumber(width) or 400;
	height = tonumber(height) or 2;
	color = TRP3_API.GetColorFromString(color or "") or TRP3_API.Colors.WHITE;
	align = alignmentAttributes[align] or "center";

	return string.format('<P align="%1$s">|T130871:%2$d:%3$d:0:0:8:8:0:8:0:8:%4$d:%5$d:%6$d|t</P>', align, height, width, color:GetRGBAsBytes());
end
...
local structureTags = {
	...
	["{(hr[^}]*)}"] = GenerateHorizontalRuleMarkup,
};
...
Utils.str.toHTML = function(text, noColor, noBrackets)
	...
	for _, line in pairs(tab) do
		...
		-- Spacers
		line = line:gsub("{(sp[^}]*)}", GenerateIndentationMarkup);
