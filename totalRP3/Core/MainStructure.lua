...
local TRP3_TutorialFrame, TRP3_TutorialTooltip, TRP3_MainTutorialButton =
	TRP3_TutorialFrame, TRP3_TutorialTooltip, TRP3_MainTutorialButton
local TRP3_MainFrameMenuContainer, TRP3_MainFramePageContainer, TRP3_MainFrame =
	TRP3_MainFrameMenuContainer, TRP3_MainFramePageContainer, TRP3_MainFrame
...
local closeAllButton =
	CreateFrame("Button", "TRP3_MainFrameMenuButtonCloseAll", TRP3_MainFrameMenuContainer, "TRP3_CommonButton")
...
local function rebuildMenu()
	...
	for i, id in pairs(ids) do
		...
		if
			not menuStructure.isChildOf
			or menuStructure.isChildOf == selectedMenuId
			or (selectedMenuId and menuStructures[selectedMenuId].isChildOf == menuStructure.isChildOf)
		then
			local uiButton = uiMenuWidgets[index + 1]
			if uiButton == nil then -- Create the button
				uiButton = CreateFrame(
					"Button",
					"TRP3_MainFrameMenuButton" .. index,
					TRP3_MainFrameMenuContainer,
					"TRP3_CategoryButton"
				)
			...
			if
				closeableChildCount > 0
				and menuStructure.isChildOf
				and menuStructures[menuStructure.isChildOf].closeable
				and (not ids[i + 1] or not menuStructures[ids[i + 1]].isChildOf)
			then
...
local function registerMenu(menuStructure)
	...
	assert(
		not menuStructures[menuStructure.id],
		"The menu with id " .. menuStructure.id .. " has already been registered."
	)
...
local function registerPage(pageStructure)
	...
	assert(
		pageStructure.frame or (pageStructure.templateName and pageStructure.frameName),
		"pageStructure must have a frame or a templateName and a frameName field."
	)
	assert(
		not pageStructures[pageStructure.id],
		"The page with id " .. pageStructure.id .. " has already been registered."
	)
...
TRP3_API.navigation.openMainFrame = function()
	...
	TRP3_API.ui.misc.playUISound(
		(WOW_PROJECT_ID ~= WOW_PROJECT_MAINLINE) and SOUNDKIT.IG_CHARACTER_INFO_TAB or SOUNDKIT.ACHIEVEMENT_MENU_OPEN
	)
end
...
local function switchMainFrame()
	if TRP3_MainFrame:IsVisible() then
		TRP3_MainFrame:Hide()
		TRP3_API.ui.misc.playUISound(
			(WOW_PROJECT_ID ~= WOW_PROJECT_MAINLINE) and SOUNDKIT.IG_MAINMENU_CLOSE or SOUNDKIT.ACHIEVEMENT_MENU_CLOSE
		)
	else
...
local function showTutorial(tutorialStructure)
	...
	for frameIndex, frameInfo in pairs(tutorialStructure) do
		...
		buttonWidget:SetPoint(
			frameInfo.button.anchor,
			buttonWidget.box,
			frameInfo.button.anchor,
			frameInfo.button.x,
			frameInfo.button.y
...
local function onTutorialRefresh(pageID)
	if currentPageId == pageID then
		...
		if
			...
			and currentPage.tutorialProvider
			and currentPage.tutorialProvider()
		then
...
TRP3_API.navigation.init = function()
	TRP3_MainFrame.Close:SetScript("OnClick", function()
		switchMainFrame()
	end)
