...
local setTooltipForFrame, setTooltipForSameFrame =
	TRP3_API.ui.tooltip.setTooltipForFrame, TRP3_API.ui.tooltip.setTooltipForSameFrame
...
CopyTextPopup.CopyText:HookScript("OnEnterPressed", function()
	CopyTextPopup:Hide()
end)
CopyTextPopup.CopyText:HookScript("OnEscapePressed", function()
	CopyTextPopup:Hide()
end)
...
function TRP3_API.popup.showCopyDropdownPopup(copyTexts, customText, customShortcutInstructions)
	if type(copyTexts) ~= "table" or #copyTexts == 0 then
		return
	end
	...
	popupText = popupText
		.. customShortcutInstructions:format(
			Ellyb.ColorManager.ORANGE(Ellyb.System.SHORTCUTS.COPY),
			Ellyb.ColorManager.ORANGE(Ellyb.System.SHORTCUTS.PASTE)
		)
...
local function decorateMusic(lineFrame, musicID)
	...
	if musicDefaultName == musicShortName then
		tooltipContent = ("|cff00ff00%s: %ss\n\n|cffff9900%s: |cffffffff%s\n|cffff9900%s: |cffffffff%s"):format(
			loc.UI_MUSIC_DURATION,
			floor(musicDuration + 0.5),
			loc.CM_L_CLICK,
			loc.REG_PLAYER_ABOUT_MUSIC_SELECT2,
			loc.CM_R_CLICK,
			loc.REG_PLAYER_ABOUT_MUSIC_LISTEN
		)
	else
		tooltipContent = ("|cffffff00%s: %s\n|cff00ff00%s: %ss\n\n|cffff9900%s: |cffffffff%s\n|cffff9900%s: |cffffffff%s"):format(
			loc.UI_MUSIC_ALTTITLE,
			musicDefaultName,
			loc.UI_MUSIC_DURATION,
			floor(musicDuration + 0.5),
			loc.CM_L_CLICK,
...
local function filteredMusicBrowser()
	...
	}, filteredMusicList, TRP3_MusicBrowserContentSlider)
end
...
local function initMusicBrowser()
	...
	for line = 0, 8 do
		local lineFrame =
			CreateFrame("Button", "TRP3_MusicBrowserButton_" .. line, TRP3_MusicBrowserContent, "TRP3_MusicBrowserLine")
...
local function decorateIcon(icon, index)
	...
	setTooltipForFrame(
		...
		0,
		-100,
		Utils.str.icon(filteredIconList[index], 75),
		filteredIconList[index]
	)
	icon.index = index
end
...
local function filteredIconBrowser()
	...
	}, filteredIconList, TRP3_IconBrowserContentSlider)
end
	...
	for row = 0, 5 do
		for column = 0, 7 do
			local button = CreateFrame(
				"Button",
				"TRP3_IconBrowserButton_" .. row .. "_" .. column,
				ui_IconBrowserContent,
				"TRP3_IconBrowserButton"
			)
...
local function decorateCompanion(button, index)
	...
	setTooltipForFrame(button, TRP3_CompanionBrowser, "RIGHT", 0, -100, "|T" .. icon .. ":40|t " .. name, text)
	button.index = index
end
...
local function filteredCompanionBrowser()
	...
	}, filteredCompanionList, TRP3_CompanionBrowserContentSlider)
end
...
local function initCompanionBrowser()
	...
	for row = 0, 5 do
		for column = 0, 7 do
			local button = CreateFrame(
				"Button",
				"TRP3_CompanionBrowserButton_" .. row .. "_" .. column,
				ui_CompanionBrowserContent,
				"TRP3_IconBrowserButton"
			)
			button:ClearAllPoints()
	...
	setTooltipForSameFrame(
		TRP3_CompanionBrowserFilterHelp,
		"TOPLEFT",
		0,
		0,
		loc.UI_COMPANION_BROWSER_HELP,
		loc.UI_COMPANION_BROWSER_HELP_TT
	)
...
table.sort(COLOR_PRESETS_CLASS, function(a, b)
	return a.TX < b.TX
end)
...
table.sort(COLOR_PRESETS_RESOURCES, function(a, b)
	return a.TX < b.TX
end)
...
local function colorPresetsDropDownSelection(hexValue)
	...
	elseif hexValue == "RENAME" then
		local existingPreset, index =
			getPresetForColor(Color(TRP3_ColorBrowser.red, TRP3_ColorBrowser.green, TRP3_ColorBrowser.blue))
		TRP3_API.popup.showTextInputPopup(loc.BW_CUSTOM_NAME .. "\n\n" .. loc.BW_CUSTOM_NAME_TT, function(text)
...
local function initColorBrowser()
	...
	TRP3_ColorBrowserSelect:SetScript("OnClick", function()
		...
		if TRP3_ColorBrowser.callback ~= nil then
			TRP3_ColorBrowser.callback(
				(TRP3_ColorBrowser.red or 1) * 255,
				(TRP3_ColorBrowser.green or 1) * 255,
				(TRP3_ColorBrowser.blue or 1) * 255
			)
		end
	end)
...
local function filteredImageBrowser()
	...
	}, filteredImageList, TRP3_ImageBrowserContentSlider)
end
...
function TRP3_API.popup.showPopup(popupID, popupPosition, popupArgs)
	...
	if popupPosition and popupPosition.parent then
		...
		popup.frame:SetPoint(
			popupPosition.point or "CENTER",
			popupPosition.parent,
			popupPosition.parentPoint or "CENTER",
			0,
			0
		)
