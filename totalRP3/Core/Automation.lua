	...
	if field == "" or field == "help" then
		...
		local examples = {
			...
			string.join(
				" ",
				stem,
				WrapTextInColorCode("currently", ARG1_COLOR),
				WrapTextInColorCode(L.SLASH_CMD_SET_HELP_EXAMPLE1, ARG3_COLOR)
			),
			string.join(
				" ",
				stem,
				WrapTextInColorCode("title", ARG1_COLOR),
				WrapTextInColorCode("[form:1]", ARG2_COLOR),
				WrapTextInColorCode(L.SLASH_CMD_SET_HELP_EXAMPLE2, ARG3_COLOR)
			),
			string.join(
				" ",
				stem,
				WrapTextInColorCode("classcolor", ARG1_COLOR),
				WrapTextInColorCode("#ff0000", ARG3_COLOR)
			),
		}
		...
		SendSystemMessage(
			string.format(
				L.SLASH_CMD_HELP_FIELDS,
				WrapTextInColorCode(table.concat(self:GetFieldSetterNames(), LIST_DELIMITER), ARG1_COLOR)
			)
		)
