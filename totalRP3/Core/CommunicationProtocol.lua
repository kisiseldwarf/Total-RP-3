...
--[[ Deprecated ]]
local function CTLToChompPriority(priority)
...
local function sendObject(prefix, object, channel, target, priority, messageToken, useLoggedMessages, queue)
	if not tContains(VALID_CHANNELS, channel) then
		...
		channel, target, priority, messageToken, useLoggedMessages, queue =
			"WHISPER", channel, target, priority, messageToken, useLoggedMessages
	end
	if tContains(VALID_PRIORITIES, target) then
		-- if target has values expected for priority, bump everything back by one
		target, priority, messageToken, useLoggedMessages, queue =
			nil, target, priority, messageToken, useLoggedMessages
	end
	...
	if not TRP3_API.register.isIDIgnored(target) then
		...
		Chomp.SmartAddonMessage(PROTOCOL_PREFIX, messageToken .. serializedData, channel, target, {
			priority = priority,
...
Ellyb.Documentation:AddDocumentationTable("TotalRP3_Communication", {
	...
	Functions = {
			...
			Returns = {
				{
					...
					Type = "number",
					Nilable = false,
					Documentation = { "The message token can then be passed to sendObject to be used explicitly" },
				},
			},
		},
		{
			...
			Arguments = {
				{
					...
					Nilable = false,
					Documentation = {
						"A unique prefix for this sub-system, used to differentiate between all sub-systems using communications. An error will be thrown if the prefix has already been registered before.",
					},
				},
				{
					Name = "callback",
					Type = "function",
					Nilable = false,
					Documentation = { "This callback will be called when we receive data that uses this prefix." },
				},
			},
		},
		{
			...
			Type = "Function",
			Documentation = {
				"Register a new callback for a specific message token to be called when we receive new data for this message token.",
			},
			...
			Arguments = {
				{
					...
					Type = "string",
					Nilable = false,
					Documentation = { "A valid message token generated using getNewMessageToken()." },
				},
				{
					Name = "sender",
					Type = "string",
					Nilable = false,
					Documentation = {
						"The unit name for the sender of the messages that we will be receiving on the message token; It is used to filter out other sender and only listen to messages coming from the expected sender.",
					},
				},
				{
					...
					Nilable = false,
					Documentation = {
						'This callback will be called when we receive new data for the specified message token with "sender, amountOfMessagesIncoming, amountOfMessagesReceived" as arguments..',
					},
				},
			},
			...
			Returns = {
				{
					...
					Nilable = false,
					Documentation = {
						"An identifier for the callback registered, to be used later for unregisterMessageTokenProgressHandler().",
					},
				},
			},
		},
		{
			...
			Type = "Function",
			Documentation = {
				"Unregister an existing callback for a specified message token. It is best practice to unregister callbacks once we don't need them anymore.",
			},
			...
			Arguments = {
				{
					...
					Nilable = false,
					Documentation = {
						"The identifier for the callback we want to unregister, as received when registering the callback.",
					},
				},
			},
		},
		{
			...
			Arguments = {
				{
					...
					Type = "table",
					Nilable = false,
					Documentation = { "The structure that would be sent." },
				},
				{
					Name = "shouldBeCompressed",
					Type = "boolean",
					Nilable = true,
					Documentation = {
						"Indicates if the structure should be compressed to reduce its size before sending.",
					},
				},
			},
			...
			Returns = {
				{
					Name = "approximateAmountOfMessages",
					Type = "number",
					Nilable = false,
					Documentation = { "The approximate amount of messages need to send the structure." },
				},
			},
		},
		{
			...
			Type = "Function",
			Documentation = {
				"Extract the message token from data received from Chomp, to get proper deserializable data and the message token separately.",
			},
			...
			Arguments = {
				{
					...
					Type = "string",
					Nilable = false,
					Documentation = { "A string of data received from Chomp." },
				},
			},
			...
			Returns = {
				{
					...
					Nilable = false,
					Documentation = {
						"The internal message token used by Total RP 3 to keep track of a single structure being sent over several messages.",
					},
				},
				{
					Name = "data",
					...
					Nilable = false,
					Documentation = {
						"The actual raw data that was sent by Total RP 3. Can be deserialize using Chomp's Deserialize function. MUST be decompress using TRP3's Compression module if the message came from non-logged channels.",
					},
				},
			},
		},
...
TRP3_API.communication.getMessageIDAndIncrement = Ellyb.DeprecationWarnings.wrapFunction(
	getNewMessageToken,
	"TRP3_API.communication.getMessageIDAndIncrement",
	"AddOn_TotalRP3.Communications.getNewMessageToken"
)
TRP3_API.communication.registerProtocolPrefix = Ellyb.DeprecationWarnings.wrapFunction(
	registerSubSystemPrefix,
	"TRP3_API.communication.registerProtocolPrefix",
...
Ellyb.DeprecationWarnings.wrapAPI(
	AddOn_TotalRP3.Communications,
	"TRP3_API.communication",
	"AddOn_TotalRP3.Communications",
	TRP3_API.communication
