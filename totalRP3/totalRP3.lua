...
local function loadingSequence()
	...
	TRP3_API.events.fireEvent(
		TRP3_API.events.NAVIGATION_RESIZED,
		TRP3_MainFramePageContainer:GetWidth(),
		TRP3_MainFramePageContainer:GetHeight()
	)
...
function TRP3_ShowErrorMessage()
	print(
		COLORS.ORANGE(("[TRP3: %s]"):format(TRP3_API.VERSION_DISPLAY))
			.. " "
			.. COLORS.RED("Error during addon loading sequence:")
	)
	...
	print(
		COLORS.ITEM_ARTIFACT(
			"Note: If you just recently updated the add-on, remember that you need to fully relaunch the game client. Updating while the game client is open will not load new files."
		)
	)
end
